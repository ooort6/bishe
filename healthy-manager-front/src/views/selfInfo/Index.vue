<template>
  <div style="display: flex;flex-direction: column;align-items: center;width: 100%;">
    <el-row type="flex"  justify="end" style="margin: 20px;">
        <el-button type="primary" size="medium" @click="handleToAnalysis">查看体检数据分析</el-button>
    </el-row>
...
export default {
  ...
  methods: {
    ...
    async getDataAnalysis() {
      let user = JSON.parse(localStorage.getItem("user"));
      let { data: checkData } = await this.$http.get("/checkInfo/getCurrentCheckInfo/" + user.id);
      this.heightData = checkData.height;
      this.weightData = checkData.weight;
      this.$http
        .get("/checkInfo/getBim", {
          params: {
      ...
      this.$http
        .get("/checkInfo/getDataAnalysis/" + user.id)
        .then((res) => {
          if (res.status === 200) {
