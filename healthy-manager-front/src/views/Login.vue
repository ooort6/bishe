  ...
  <div class="login" v-loading="loading">
    <el-card style="background-color: rgba(255, 255, 255, 0.8)">
      ...
      <el-form :model="loginForm" :rules="rules" ref="loginForm">
        <h1 style="margin: 4px 0">大学生健康管理系统</h1>
        <span
          style="
            font-size: 20px;
            letter-spacing: 20px;
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="success" @click="register(registerForm)"
          >确 定</el-button
        >
      </span>
    </el-dialog>
  ...
  data() {
    ...
    return {
      ...
      rules2: {
        ...
        StuNo: [{ validator: validateUsername, trigger: "blur" }],
      },
      dialogVisible: false,
      registerForm: {
        username: "",
  ...
  methods: {
    login(data) {
      ...
      if (data.username === "" || data.password === "") {
        ...
        self.$http
          .post("/user/login", self.loginForm)
          .then((res) => {
            ...
            if (res.status === 200) {
              ...
              //缓存当前用户的健康档案
              self.$http
                .get("/checkInfo/getCurrentCheckInfo/" + user.id)
                .then((res) => {
                  if (res.status === 200) {
              ...
              self.$http
                .get("/resources/getMenuByUserId", {
                  params: { userId: user.id, typeId: 0 },
                })
                .then((res) => {
                  if (res.status === 200) {
                    // 添加智能健康分析模块到菜单
...
.login {
  ...
  width: 100%;
  height: calc(~"100vh");
  background: url("../assets/hbueSchool.jpg");
